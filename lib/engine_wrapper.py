...
def get_homemade_engine(name: str) -> type[MinimalEngine]:
    """
    Get the homemade engine with name `name`. e.g. If `name` is `RandomMove`
    then we will return `homemade.RandomMove`. If not found, check minmax_bot.py
    ...
    try:
        ...
        # Fallback: load SmartBot (or other custom homemade bots) from minmax_bot.py
        try:
            from minmax_bot import SmartBot
            if name == "SmartBot":
                return SmartBot
            raise AttributeError(f"No class named {name} found in minmax_bot.py")
        except Exception as e:
            raise ImportError(
                f"Unable to load homemade engine '{name}'. "
                f"Tried homemade.py and minmax_bot.py. Original error: {e}"
            )
